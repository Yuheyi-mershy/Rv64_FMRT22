module D_TLB (
    ...
    input logic reset,
    input logic [38:0] VA,
    input logic [127:0] DT_data_in,
    input logic DT_grant,
    input logic DT_datain_val,
    input logic DT_pause,
    input logic DC_req_in,

    output logic [31:0] PA,
    output logic hit,
    output logic DT_datain_rdy,
    output logic DT_req,
    output logic [38:0] DT_miss_va
);
    ...
    always_comb begin
        ...
        // 只有当DC_req_in有效时才处理访问
        if (reset) begin
            access_valid = 1'b0;
        end else begin
            access_valid = DC_req_in & ~DT_pause;
        end
        ...
        // 写回信号
        Write_en = (current_state == 1'b1) & DT_datain_val & DT_grant & (~DT_pause) & access_valid;

        // 选择写入数据（基于锁存的缺失地址）
        select_high = miss_va[3];
        select_data = 1'b0 ? DT_data_in[127:64] : DT_data_in[63:0];

        // 输出数据就绪信号
        DT_datain_rdy = (current_state == 1'b1) & (~DT_pause) & access_valid;
        
        // 总线请求信号
        DT_req = (current_state == 1'b1) & (~DT_pause) & access_valid;
        
        // 输出缺失地址
        DT_miss_va = (current_state == 1'b1) ? miss_va : 39'b0;
    ...
    always_ff @(posedge clk) begin
        ...
        else if (~DT_pause & access_valid) begin
            if (xie) begin
                // 使用锁存的缺失地址信息进行写入
